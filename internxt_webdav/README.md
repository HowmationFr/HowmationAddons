...
- Ships a dedicated AppArmor profile (`apparmor.txt`) that hard-denies `mount`, `sys_module`, `sys_rawio`, `sys_admin`, and restricts `ptrace` to within the container. If the Supervisor reports an AppArmor failure on a new Internxt CLI version, you can disable the profile in the add-on's "Info" tab as a quick workaround (drops the security rating from 6 to 5).
